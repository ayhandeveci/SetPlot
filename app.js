...
// Returns last performance for an exercise in a given day type (A/B/C)
function getLastPerformance(exercise, dayType) {
  const history = loadHistory();
  const relevant = history
    .filter(l => l.exercise === exercise && l.workout_day && l.workout_day.startsWith(dayType))
...
function calcSuggestion(last) {
  ...
  const kg   = last.actual_kg;
  const reps = last.actual_reps;
  const rir  = last.rir;

  // Minimum = last performance as-is
  const minimum = { kg, reps };

  // No RIR recorded → only show minimum
  if (rir === "" || rir === null || rir === undefined) {
    return { minimum, suggestion: null };
  }

  let sKg = kg, sReps = reps;
  ...
  // rir >= 3 → same kg, same reps (still has room)
...
function renderSummary() {
  ...
  $("summaryTitle").textContent    = workouts[state.selectedDay].label + " Summary";
